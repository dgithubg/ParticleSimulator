...
void myMouse(int button, int state, int x, int y)
{
	...
	partSys* ps = (partSys*)GlobalResourceManager::use()->getSystem( "partSys" );

	GlobalResourceManager::use()->setMouseButtonInfo( button, state );
	GLint pickedObject;
	if( button == GLUT_LEFT_BUTTON && state == GLUT_DOWN )
	{
		ps->pickObject(x, y, &pickedObject);
		int s = ps->selectedIndex();
		if(s >= 0)
			animTcl::OutputMessage("Particle Selected %d\n",s);
		else
			animTcl::OutputMessage("No object was clicked\n");
...
void myMotion(int x, int y)
{
	...
	{
		// set position of selected particle to (x, y, former z)
		ps->setPartPos(x, y);
		animTcl::OutputMessage(
...
void MakeScene(void)
{
	...
	*/
	...
	// register a system
	partSys* particleSystem = new partSys( "partSys" );

	success = GlobalResourceManager::use()->addSystem( particleSystem, true );

	// make sure it was registered successfully
	assert( success );

	// register a simulator
	partSim* particleSimulator = 
		new partSim( "partSim", particleSystem );
	...
	assert( partSysRetrieval );

	BaseSimulator* partSimRetrieval;
